...
test_that("Multi-level Geometric example from Morningstar methodology paper" , {
  ...
  expect_equal(attribution_results$`Allocation at each level`$`Level 1`[2, "Asia"], 0.0016, tolerance = epsilon)
  expect_equal(attribution_results$`Allocation at each level`$`Level 1`[2, "Europe"], 0.0013, tolerance = epsilon)
  
  expect_equal(attribution_results$`Allocation at each level`$`Level 2`[2, "Asia-Cyclical"], 0.0021, tolerance = epsilon)
  expect_equal(attribution_results$`Allocation at each level`$`Level 2`[2, "Asia-Defensive"], 0.0041, tolerance = epsilon)
  expect_equal(attribution_results$`Allocation at each level`$`Level 2`[2, "Europe-Cyclical"], -0.0083, tolerance = epsilon)
  expect_equal(attribution_results$`Allocation at each level`$`Level 2`[2, "Europe-Defensive"], -0.0145, tolerance = epsilon)
  
  expect_equal(attribution_results$`Allocation at each level`$`Level 3`[2, "Asia-Cyclical-Large Cap Asia Cyclical"], -0.0139, tolerance = epsilon)
  expect_equal(attribution_results$`Allocation at each level`$`Level 3`[2, "Asia-Cyclical-Small Cap Asia Cyclical"], -0.0028, tolerance = epsilon)
  expect_equal(attribution_results$`Allocation at each level`$`Level 3`[2, "Asia-Defensive-Large Cap Asia Defensive"], -0.0083, tolerance = epsilon)
  expect_equal(attribution_results$`Allocation at each level`$`Level 3`[2, "Asia-Defensive-Small Cap Asia Defensive"], -0.0167, tolerance = epsilon)
  expect_equal(attribution_results$`Allocation at each level`$`Level 3`[2, "Europe-Cyclical-Large Cap Europe Cyclical"], -0.0021, tolerance = epsilon)
  expect_equal(attribution_results$`Allocation at each level`$`Level 3`[2, "Europe-Cyclical-Small Cap Europe Cyclical"], -0.0008, tolerance = epsilon)
