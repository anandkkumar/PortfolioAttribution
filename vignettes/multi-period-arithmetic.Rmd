...
knitr::kable(multi_period_portf_2$wp, caption = "Portfolio Weights")
knitr::kable(multi_period_portf_2$Rp, caption = "Portfolio Returns")
knitr::kable(multi_period_portf_2$wb, caption = "Benchmark Weights")
knitr::kable(multi_period_portf_2$Rb, caption = "Benchmark Returns")
```
...
cbind(attribution_results_grap$Allocation, 
      attribution_results_carino$Allocation,
      attribution_results_menchero$Allocation,
      attribution_results_frongello$Allocation,
      attribution_results_davies_laker$Allocation) %>% 
  kableExtra::kable("html", caption = "Allocation Effects") %>% 
  kableExtra::kable_styling("striped") %>% 
  kableExtra::column_spec(2:5, color = "blue") %>%
  kableExtra::column_spec(6:9, color = "orange") %>%
  kableExtra::column_spec(10:13, color = "darkcyan") %>%
  kableExtra::column_spec(14:17, color = "red") %>%
  kableExtra::column_spec(18:21, color = "purple") %>%
  # The 'striped' styling directive above does not appear to work and so we stripe if manually
  kableExtra::row_spec(seq(2, NROW(attribution_results_grap$Allocation), by=2), background = "#F7F7F7") %>% 
  kableExtra::row_spec(NROW(attribution_results_grap$Allocation), bold = TRUE) %>% 
  kableExtra::add_header_above(c(" " = 1, "GRAP" = 4, "Carino" = 4, "Menchero" = 4, "Frongello" = 4, "Davies and Laker" = 4))

cbind(attribution_results_grap$Selection, 
      attribution_results_carino$Selection,
  ...
  kableExtra::row_spec(NROW(attribution_results_grap$Selection), bold = TRUE) %>% 
  kableExtra::add_header_above(c(" " = 1, "GRAP" = 4, "Carino" = 4, "Menchero" = 4, "Frongello" = 4, "Davies and Laker" = 4))

cbind(attribution_results_davies_laker$Interaction) %>% 
  kableExtra::kable("html", caption = "Interaction Effects") %>% 
  kableExtra::kable_styling("striped") %>% 
  kableExtra::column_spec(2:5, color = "purple") %>%
  # The 'striped' styling directive above does not appear to work and so we stripe if manually
  kableExtra::row_spec(seq(2, NROW(attribution_results_davies_laker$Interaction), by=2), background = "#F7F7F7") %>% 
  kableExtra::row_spec(NROW(attribution_results_davies_laker$Interaction), bold = TRUE) %>% 
  kableExtra::add_header_above(c(" " = 1, "Davies and Laker" = 4))
...
* Counter-intuitively, the `Carino` and `Menchero` methods are sensitive to the total number of periods used to smooth the effects. In other words, if you lengthen the period of analysis, you will have to recompute the effects for __all periods__ and not just the additional ones added. 
