...
Attribution.levels(Rp, wp, Rb, wb, h = h, h_levels = c("type", "MarketCap", "currency", "Sector"), geometric = TRUE, anchored = FALSE)
Attribution.levels(Rp, wp, Rb, wb, h = h, h_levels = c("type", "currency", "Sector"), geometric = TRUE, anchored = FALSE)
Attribution.levels(Rp, wp, Rb, wb, h = h, h_levels = c("type", "Sector"), geometric = TRUE, anchored = FALSE)

PerformanceAnalytics::Return.annualized.excess(rp, rb)
...
rp = xts::reclass(rowSums(Rp * wp), Rp)
rb = xts::reclass(rowSums(Rb * wb), Rb)
rp = PerformanceAnalytics::checkData(rp)
rb = PerformanceAnalytics::checkData(rb)
...
RP=Rp[1, ]
RB=Rb[1, ]
WB=as.vector(zoo::coredata(wb[1, ]))
WP=as.vector(zoo::coredata(wp[1, ]))
Attribution.geometric(RP, WP, RB, WB) # The same as in Table 5.5

# 6.2 Multi-period
zoo::index(wp) <- as.Date(c("2011-12-31", "2012-02-01", "2012-03-01", "2012-04-01"))
zoo::index(wb) <- as.Date(c("2011-12-31", "2012-02-01", "2012-03-01", "2012-04-01"))
